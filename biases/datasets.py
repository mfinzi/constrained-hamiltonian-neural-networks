...
class RigidBodyDataset(Dataset,metaclass=Named):
    ...
    def __init__(self, root_dir=None,body=ChainPendulum(3), n_systems=100, regen=False,
                 chunk_len=5,dt=.1,integration_time=50):
        super().__init__()
        root_dir = root_dir or os.path.expanduser(f'~/datasets/ODEDynamics/{self.__class__}/')
        self.body = body
        filename = os.path.join(root_dir, f"trajectories_N{n_systems}_dt{dt}_T{integration_time}.pz")
    ...
    def generate_trajectory_data(self, n_systems, dt,integration_time, bs=100):
        ...
        while n_gen < n_systems:
            ...
            ts = torch.arange(0,integration_time,dt,device=z0s.device,dtype=z0s.dtype)
