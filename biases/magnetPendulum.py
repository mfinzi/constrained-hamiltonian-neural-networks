...
from biases.chainPendulum import PendulumAnimation, ChainPendulum
import numpy as np
...
class MagnetPendulum(RigidBody):
    def __init__(self, mass=1, l=1,q=1,magnets=4):
        ...
        self.magnet_positions = torch.stack([.1*theta.cos(),.1*theta.sin(),-(1.2)*l*torch.ones_like(theta)],dim=-1)
    ...
    def sample_initial_conditions(self,N):
        ...
        xv = .2*torch.randn(N, 2, 1, 3)  # (N,2,n,d)
        xv[:,0,:,:]+=torch.tensor([0.,0.,-1.])
        xv[:,0,:,:]/=(xv[:,0,:,:]**2).sum(-1,keepdims=True).sqrt()
        
        xv[:,1,:,:] -= xv[:,0,:,:]*(xv[:,0,:,:]*xv[:,1,:,:]).sum(-1,keepdims=True)
        #xv[:,1,:,:]*= .1
        return xv
    ...
    def potential(self, x):
        ...
        dipole_energy = 3*(m0dotr0i*midotr0i-r0inorm2*m0dotmi)/(4*np.pi*r0inorm2**(5/2)) # (bs,magnets)
...
class MagnetPendulumAnimation(PendulumAnimation):
    def __init__(self,*args,**kwargs):
        ...
        if d==3: self.magnets.set_3d_properties(self.body.magnet_positions[:,2].data.numpy())
        self.ax.set_xlim((-1,1))
        self.ax.set_ylim((-1,1))
        if d==3: self.ax.set_zlim((-1.3,0))
